...
export async function detectEvmChain(address: string): Promise<Chain> {
  ...
  try {
    ...
    // If only one has balance, use that chain
    if (ethBalance > 0 && baseBalance === 0) return "ethereum";
    if (baseBalance > 0 && ethBalance === 0) return "base";

    // If both have balance, use the one with more (likely more active)
    if (ethBalance > 0 && baseBalance > 0) {
      return baseBalance > ethBalance ? "base" : "ethereum";
    }

    // Neither has balance — default to ethereum
    return "ethereum";
  } catch {
    // On any error (timeout, network), default to ethereum
    return "ethereum";
  } finally {
...
async function fetchNativeBalance(
  ...
  try {
    ...
    // Convert hex wei to a comparable number
    // Using Number is fine — we only need relative comparison, not precision
